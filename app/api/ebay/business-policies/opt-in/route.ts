...
export async function POST() {
  try {
    ...
    // Surface the real upstream error (status + endpoint + body) to the UI.
    return NextResponse.json(
      {
        ok: false,
        error: e?.message || "Opt-in failed",
        hint:
          "If this mentions insufficient scope or 401/403, reconnect eBay so the token includes sell.account scope.",
      },
      { status: 500 }
    );
  }
